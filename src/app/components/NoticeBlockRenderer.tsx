...
export default function NoticeBlockRenderer({ block }: NoticeBlockRendererProps) {
  switch (block.type) {
    case "paragraph":
      return (
        <Typography
          variant="body1"
          component="p"
          sx={{ color: "text.secondary", ...(block.emphasis ? emphasisSx[block.emphasis] : {}) }}
        >
          {block.text}
        </Typography>
      );
    ...
    case "callout":
      return <CalloutBox tone={block.tone} title={block.title} lines={block.lines} />;
    ...
    case "numbered":
      return (
        <List component="ol" disablePadding sx={{ listStyle: "none", m: 0, p: 0 }}>
          {block.items.map((item, i) => (
            <ListItem key={i} disableGutters sx={{ py: 0.35, display: "flex", gap: 1 }}>
    ...
    case "linkBox":
      return <LinkBox label={block.label} href={block.href} items={block.items} image={block.image} />;
